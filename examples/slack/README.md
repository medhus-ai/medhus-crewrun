...
`.crew/memory/ai-runners.json` files. Configure the Slack gateway with Slack IDs, not display
names:
...
Slack → signature + replay check → configured user check → event-id dedupe
      → immediate 200 acknowledgement → asynchronous role turn → chat.postMessage in thread
```
...
`createSlackPoster`) are injectable for tests or a different host transport.
